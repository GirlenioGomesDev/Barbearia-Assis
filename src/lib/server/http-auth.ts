  ...
  const status =
    name === "RateLimit"
      ...
      : name === "Unauthorized"
        ...
        : name === "CsrfError"
          ? 403
          : name === "RevisionConflict" || name === "ImageInUse"
            ? 409
            : name === "StorageUnavailable"
              ? 503
              : name === "EnvironmentError"
                ? 503
                : name === "ValidationError" || name === "InvalidImageDelete"
                  ? 422
                  : 500;
...
export function verifyHttpPassword(
  password: string,
  storedHash = runtimeValue("ADMIN_PASSWORD_HASH"),
) {
...
export function hashHttpPassword(password: string, salt = randomBytes(16).toString("hex")) {
  return `scrypt:${salt}:${scryptSync(password, salt, 64).toString("hex")}`;
}

export function createHttpSessionHeaders(maxAgeSeconds = MAX_AGE_SECONDS) {
  ...
  headers.append("Set-Cookie", cookie(SESSION_COOKIE, signPayload(payload), true));
  headers.append("Set-Cookie", cookie(CSRF_COOKIE, csrf, false));
  return { headers, csrf };
}

export function clearHttpSessionHeaders() {
  const headers = new Headers();
  headers.append("Set-Cookie", `${SESSION_COOKIE}=; Path=/; Max-Age=0; SameSite=Lax`);
  headers.append("Set-Cookie", `${CSRF_COOKIE}=; Path=/; Max-Age=0; SameSite=Lax`);
  return headers;
}

export function getHttpSession(request: Request) {
...
export function requireHttpAdmin(request: Request) {
...
export function requireHttpCsrf(request: Request) {
  const session = requireHttpAdmin(request);
  if (isAdminAuthDisabled()) return session;
  if (request.headers.get("x-csrf-token") !== session.csrf) {
    const error = new Error("Solicitação recusada.");
...
export function isAdminAuthDisabled() {
  return runtimeValue("ENVIRONMENT") !== "production" && !runtimeValue("ADMIN_PASSWORD_HASH");
}

export function devCsrfToken() {
  return DEV_CSRF;
}

export function assertAuthConfigured() {
  if (!runtimeValue("ADMIN_PASSWORD_HASH")) {
    throw namedError("EnvironmentError", "ADMIN_PASSWORD_HASH não está configurada.");
  }
  if (runtimeValue("ADMIN_SESSION_SECRET").length < 32) {
    throw namedError(
      "EnvironmentError",
...
function cookie(name: string, value: string, httpOnly: boolean) {
  return [
    ...
    "SameSite=Lax",
    httpOnly ? "HttpOnly" : "",
    runtimeValue("ENVIRONMENT") === "production" ? "Secure" : "",
  ]
    .filter(Boolean)
    .join("; ");
}
